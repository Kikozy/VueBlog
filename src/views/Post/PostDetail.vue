...
import { onMounted, reactive, shallowRef, watch, nextTick, onUnmounted, onBeforeUnmount } from "vue"
...
onMounted(() => {
  ...
  console.log("看看当前的信息", query, params)
  if (route.query.postId) loadPostDetails(route.query.postId as string)
})

async function loadPostDetails(pid: string) {
...
rendererMD.image = (href, text) => {
  let imgId = "img" + ++postState.imgCount
